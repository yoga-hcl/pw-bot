...
(async () => {
  ...
  //allow camero permission in this browser context
  ...
  await page.locator('[placeholder="Enter name"]').fill('YogaV');

  // Click button:has-text("Enable video")
  //await page.locator('button:has-text("Enable video")').click();

  // Click text=Join now
  //await page.locator('text=Join now').click(); //click event not working while browser prompt pop-up

  //TODO: Yoga try to locate the 'Join now' button based css class and then trigger click event

  // Click [aria-label="Hang up"]
  //await Promise.all([
  //  page.waitForNavigation(/*{ url: 'https://teams.microsoft.com/_#/conversations/19:meeting_ZTVjMzlmMGEtMmZhMS00NDk0LTgyNTktMGFmOWNjNjI0YTY5@thread.v2?ctx=chat' }*/),
  ...
  // Click [aria-label="Close sharing options"]
  //await page.locator('[aria-label="Close sharing options"]').click();
